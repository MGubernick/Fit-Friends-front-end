...
class LowerBodyIndex extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    ...
    indexAllWorkouts(user)
      ...
      .then(() => msgAlert({
        message: 'Check it out! Here are all of the Lower Body workouts!',
        variant: 'success'
      }))
      .catch(error => {
        msgAlert({
          heading: 'Showing The Lower Body Workouts Failed',
  ...
  render () {
    ...
    const lBWorkouts = workouts.filter(workout => workout.category === 'Lower Body')

    workoutJsx = lBWorkouts.map(workout => (
      <Card key={workout.id}
        onClick={(event) => {
        ...
        <Card.Body>
          ...
          <div style={{ alignContent: 'center', display: 'flex', justifyContent: 'center' }}>
            <Card.Img src={'https://imgur.com/VKDfplf.png'} style={{ alignSelf: 'center', height: '80px', width: '80px' }} alt='image of a muscle'/>
          </div>
        </Card.Body>
      </Card>
    ))
    ...
    return (
      <div style={{ alignContent: 'center', display: 'flex', flexDirection: 'column' }}>
        <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2>Lets take a look at our library of lower body workouts!</h2>
...
export default withRouter(LowerBodyIndex)
