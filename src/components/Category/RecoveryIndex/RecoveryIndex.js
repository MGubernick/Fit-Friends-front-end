...
class RecoveryIndex extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    ...
    indexAllWorkouts(user)
      ...
      .then(() => msgAlert({
        message: 'Check it out! Here are all of the recovery workouts!',
        variant: 'success'
      }))
      .catch(error => {
        msgAlert({
          heading: 'Showing The Recovery Workouts Failed',
  ...
  render () {
    ...
    const recoveryWorkouts = workouts.filter(workout => workout.category === 'Recovery')

    workoutJsx = recoveryWorkouts.map(workout => (
      <Card key={workout.id}
        onClick={(event) => {
        ...
        <Card.Body>
          ...
          <div style={{ alignContent: 'center', display: 'flex', justifyContent: 'center' }}>
            <Card.Img src={'https://imgur.com/sgWB0ro.png'} style={{ alignSelf: 'center', height: '80px', width: '80px' }} alt='image of a muscle'/>
          </div>
        </Card.Body>
      </Card>
    ))
    ...
    return (
      <div style={{ alignContent: 'center', display: 'flex', flexDirection: 'column' }}>
        <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2>Lets take a look at our library of Recovery workouts!</h2>
...
export default withRouter(RecoveryIndex)
