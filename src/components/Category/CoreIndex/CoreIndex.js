...
class CoreIndex extends Component {
  ...
  componentDidMount () {
    ...
    indexAllWorkouts(user)
      ...
      .then(() => msgAlert({
        message: 'Check it out! Here are all of the core workouts!',
        variant: 'success'
      }))
      .catch(error => {
        msgAlert({
          heading: 'Showing The Core Workouts Failed',
  ...
  render () {
    ...
    const coreWorkouts = workouts.filter(workout => workout.category === 'Core')

    workoutJsx = coreWorkouts.map(workout => (
      <Card key={workout.id}
        onClick={(event) => {
        ...
        <Card.Body>
          ...
          <div style={{ alignContent: 'center', display: 'flex', justifyContent: 'center' }}>
            <Card.Img src={'https://imgur.com/IQoO7qe.png'} style={{ alignSelf: 'center', height: '80px', width: '80px' }} alt='image of a muscle'/>
          </div>
        </Card.Body>
      </Card>
    ))
    ...
    return (
      <div style={{ alignContent: 'center', display: 'flex', flexDirection: 'column' }}>
        <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2>Lets take a look at our library of core workouts!</h2>
...
export default withRouter(CoreIndex)
